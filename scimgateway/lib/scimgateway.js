...
const isMethodAllowed = (array, method) => {
  return array ? array.includes(method) : true
}
...
try{
    ...
    return new Promise((resolve, reject) => { // basic auth
      ...
      for (let i = 0; i < arr.length; i++) {
        if (arr[i].username === userName && arr[i].password === userPassword) { // authentication OK
          ...
          if(!isMethodAllowed(arr[i].allowedMethods, method)) return reject(new Error(`method not allowed for user ${arr[i].username}`))
    ...
    return new Promise((resolve, reject) => { // bearer token
      ...
      for (let i = 0; i < arr.length; i++) {
        if (arr[i].token === authToken) { // authentication OK
          ...
          if(!isMethodAllowed(arr[i].allowedMethods, method)) return reject(new Error(`method not allowed for token`))
        ...
        if (user) { // authenticated OK
          ...
          for (let i = 0; i < arr.length; i++) {
            if (arr[i].tenantIdGUID && payload.iss.includes(arr[i].tenantIdGUID)) {
              ...
              if(!isMethodAllowed(arr[i].allowedMethods, method)) return reject(new Error(`method not allowed for user ${arr[i].tenantIdGUID}`))
    ...
    return new Promise((resolve, reject) => {
      jwt.verify(authToken, (el.secret) ? el.secret : el.publicKeyContent, el.options, (err, decoded) => {
        if (err) resolve(false)
          ...
          if(!isMethodAllowed(arr[i].allowedMethods, method)) return resolve(false)
  ...
  const bearerOAuth = (baseEntity, method, authType, authToken) => {
    return new Promise((resolve, reject) => { // bearer token
      ...
      if (config.auth.oauthTokenStore[authToken]) { // authentication OK
        ...
        if(!isMethodAllowed(arr[i].allowedMethods, method)) return reject(new Error(`method not allowed for this bearerOAuth`))
  ...
  const authPassThrough = async (baseEntity, method, authType, authToken, ctx) => {
    ...
    if(!isMethodAllowed(arr[i].allowedMethods, method)) return reject(new Error(`method not allowed for passThrough`))
  ...
  app.use(async (ctx, next) => {
    ...
    console.log(`${method} - ${ctx.request.path}`)
      ...
      async function runInterceptors(){
        let res = await fetchInterceptors(config, ctx, caches, verifyTracing, ScimGateway.prototype.endpointMapper)
        if(res){
          resolve(next());
        } else{
          resolve(ctx)
  ...
  app.use(async (ctx, next) => {
    return new Promise(async(resolve) => {
      async function runAdapters(){        
        const notificationRes = await fetchAdapters(config, ctx, 'before', caches, verifyTracing)
        if(notificationRes){
          resolve(next());  
        } else{
          resolve(ctx)
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans|Entitlements)/:id`], async (ctx) => {
    ...
    try {
      ...
      ctx.body = scimdata
      await fetchAdapters(config, ctx, 'onSuccess', caches, verifyTracing)
    } catch (err) {
      let scimType = errorStatus(err, isScimv2, ctx)
      const e = jsonErr(config.scim.version, pluginName, ctx.status, err)
      ctx.body = e
      await fetchAdapters(config, ctx, 'onError', caches, verifyTracing)
    }
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans|Entitlements)/:id`], async (ctx) => {
    ...
    if (strBody === '{}') {
      ...
      try {
        ...
        ctx.status = 200
        ctx.body = scimdata
        await fetchAdapters(config, ctx, 'onSuccess', caches, verifyTracing, ScimGateway.prototype.convertedScim20(ctx.request.body))
      } catch (err) {
        let scimType = errorStatus(err, isScimv2, ctx)
        const e = jsonErr(config.scim.version, pluginName, ctx.status, err, scimType)
        ctx.body = e
        await fetchAdapters(config, ctx, 'onError', caches, verifyTracing, ScimGateway.prototype.convertedScim20(ctx.request.body))
      }
    }
    endSpan(ctx, span, requestSpan)
  })
...
const errorStatus = (err, isScimv2, ctx) => {
  ...
  if (err.name) {
    ...
    if (err.name === "notFound") ctx.status = 404; // Not found
      ...
      if (err.message && err.message.toLowerCase().includes("duplicate key") || err.message.toLowerCase().includes("already exists")) {
